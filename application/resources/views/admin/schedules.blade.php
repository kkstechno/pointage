@extends('layouts.default')
    ...
    @section('meta')
        <title>Horaires de travail | KKS-Presence</title>
    ...
    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">{{ __('HEURE DE TRAVAIL ') }}
                <button class="ui positive button mini offsettop5 btn-add float-right"><i class="ui icon plus"></i>{{ __('Ajouter') }}</button>
            </h2>
        </div>
                ...
                <div class="box-body">
                    <table width="100%" class="table table-striped table-hover" id="dataTables-example" data-order='[[ 6, "asc" ]]'>
                        <thead>
                            <tr>
                                <th>{{ __('Employé') }}</th>
                                <th>{{ __('Heure de travail') }}</th>
                                <th>{{ __('Taux horaire') }}</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $sched->hours }} H</td>
                                <td>{{ $sched->restday }}</td>
                                <td>@php echo e(date('d/m/Y', strtotime($sched->datefrom))) @endphp</td>
                                <td>@php echo e(date('d/m/Y', strtotime($sched->dateto))) @endphp</td>
                                <td>
                                    @if($sched->archive == '0') 
                                        <span class="green">{{ __('En cours') }}</span>
                                    @else
                                        <span class="teal">{{ __('Terminé') }}</span>
                                    @endif
                                </td>
